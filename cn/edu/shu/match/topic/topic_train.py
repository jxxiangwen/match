...
from cn.edu.shu.match.topic.get_documents import Document
...
def yield_document(texts, patent=False, add_list=True, participle=False):
    ...
    #             if participle:
    #                 return jieba.cut(text[1] + text[2])
    #             else:
    #                 yield text[1] + text[2]
...
#             logging.info('begin __iter__ corpus')
...
#                     if len(result) != 0:
#                         yield self.corpus_dictionary.doc2bow(
#                             yield_document(result[1] + result[2], participle=True, add_list=False))
#             else:
#                 for index in range(id_range_list_len):
#                     if index + 1 < id_range_list_len:
...
#                             if len(text) != 0:
#                                 yield self.corpus_dictionary.doc2bow(
#                                     yield_document(text[1] + text[2], participle=True, add_list=False))
...
#     if 0 != len(id_range_list):
#         if 1 == len(id_range_list):
#             # [print("SELECT {},{},{},{} FROM {} WHERE {} = {}".format(*config_json['patent_table_column_name'],
#             #                                                       config_json['patent_table'],
#             #                                                       config_json['patent_table_column_name'][0],
#             #                                                       patent_id)) for
...
#             #            patent_id in range(max_patent_id_result[0][0] + 1))
#             # print(results)
#             for result in return_result(int(config_json['gensim_dictionary_patent_max_id_used']),
#                                         int(max_patent_id_result[0][0])):
#                 if 0 != len(result):
...
#                 if 0 != len(text):
#                     yield text[1] + text[2]
#                     # return (max_require_id[0][0], max_provide_id[0][0], max_patent_id_result[0][0])
...
class TopicTrain(object):
    ...
    def update_all(self, re_train=True):
        """
        更新词典和lda模型
        :param re_train:
        :return:
        """
        if config_json['gensim_dictionary_train_used']:
            # 更新词典
            self.train_dictionary(update=True)
        else:
            # 重新训练词典
            self.train_dictionary()
        # 重新训练语料库
        self.get_corpus(True)
        # 重新训练ti-idf模型
        self.set_tf_idf()
        if re_train:
            documents = Document()
            corpus = [self.dictionary.doc2bow(text) for text in documents]
            self.re_train_or_update_lda(update=True, corpus=corpus)
        else:
            # 重新训练ti-idf模型
            self.re_train_or_update_lda(update=False)
    ...
    def train_dictionary(self, update=True):
        ...
        documents = Document(train_file=config_json['gensim_dictionary_train_used'],
                             begin_require_id=int(config_json['gensim_max_require_id_used']),
                             begin_provide_id=int(config_json['gensim_max_provide_id_used']),
                             begin_patent_id=int(config_json['gensim_dictionary_patent_max_id_used']))
        for document in documents():
            # print(document)
            self.update_dictionary_by_text(list(jieba.cut(document)))
        if not update:
    ...
    def re_train_corpus(self):
        ...
        documents = Document()
        corpus = (self.dictionary.doc2bow(yield_document(document, participle=True, add_list=False)) for document in documents)
    ...
    def re_train_or_update_lda(self, update=False, corpus=None):
        ...
        :return:
        """
        logging.info('re_train_or_update_lda')
        if not self.tf_idf_model:
            self.set_tf_idf(update)
        if not self.dictionary:
            self.get_dictionary()
    ...
    def re_train_or_update_ldamulticore(self, update=False, corpus=None):
        ...
        :return:
        """
        logging.info('re_train_or_update_ldamulticore')
        if not self.tf_idf_model:
            self.set_tf_idf(update)
        if not self.dictionary:
            self.get_dictionary()
...
if __name__ == '__main__':
    ...
    # for topic in lda.print_topics(num_topics=100, num_words=100):
    #     print(topic)
    # for id in range(10):
    ...
    #         if index > 5:
    #             break
    #         print(jieba.cut(train))
    #         print(lda.get_document_topics(dictionary.doc2bow(jieba.cut(train))))
